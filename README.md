...
The Python script `purchases_to_buckets.py` runs with the command

`python purchases_to_buckets.py`

 assuming the following conditions.

* Python is installed, as are these packages:
    * pandas
    * numpy
    * json
    * time
* The data files `purchase_buckets.csv` and `purchase_data.csv` are in the same directory as `purchases_to_buckets.py`.


**Design**

The script works by performing these tasks.

* Read purchase and bucket data into pandas DataFrames.
* Match purchases with appropriate buckets.
* Partition the buckets into the 8 (2^3) different categories according to whether the three relevant criteria (publisher, duration, and price - in that order) are wild cards or not, and assigning each category a priority level.
* Have buckets from each priority level get matched with their appropriate purchases, flagging purchases as bucketed as they go so that no purchase is bucketed twice.
* Format the results as JSON and write to file 'buckets.json'.

**Other Notes**

The script runs in about 59 seconds on my 2012 ThinkPad.
